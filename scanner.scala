...
type LoxRuntimeLiteral = Symbol | String | Double | Boolean

case class Token(
    tokenType: TokenType,
    lexeme: String,
    literal: Option[LoxRuntimeLiteral],
    line: Int
)

class Scanner(code: String):
  val keywordLookup =
    TokenType.values.map(v => v.toString.toLowerCase -> v).toMap
  ...
  def scanTokens(): List[Token] =
    ...
    def newToken(
        tokenType: TokenType,
        literal: Option[LoxRuntimeLiteral] = None
    ): Option[Token] =
    ...
    def matchNext(expected: Char): Boolean =
      if isAtEnd() || code.charAt(current) != expected then false
      else
        current += 1
        true
    ...
    def scanIdentifier(): Option[Token] =
      ...
      keywordLookup
        .get(text)
        .map(newToken(_))
        .getOrElse(newToken(TokenType.Identifier))
    ...
    def scanToken(): Option[Token] =
      ...
      c match {
        ...
        case '*' => newToken(Star)
        case '!' =>
          if matchNext('=') then newToken(BangEqual) else newToken(Bang)
        case '=' =>
          if matchNext('=') then newToken(EqualEqual) else newToken(Equal)
        case '<' =>
          if matchNext('=') then newToken(LessEqual) else newToken(Less)
        case '>' =>
          if matchNext('=') then newToken(GreaterEqual) else newToken(Greater)
        case '/' =>
          if matchNext('/') then { consumeUntilEndOfLine(); None }
          else newToken(Slash)
        case ' ' | '\r' | '\t' => None
        case '\n'              => line += 1; None
        case '"'               => scanString()
